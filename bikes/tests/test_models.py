...
def test_bike_status_available_with_user():
    ...
    with pytest.raises(ValueError, match="Невозможно создать свободный велосипед с пользователем."):
        bike.save()
...
def test_rental_event_creation(): #TODO поменять на чек функции
    ...
    rental_event = RentalEvent.objects.create(bike=bike, user=user, rented_from=rented_from, rented_until=rented_until, total_price=1000)
